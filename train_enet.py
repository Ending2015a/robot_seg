...
#Directory arguments
flags.DEFINE_string('image_dir', './dataset', '')
flags.DEFINE_string('anno_dir', './dataset', 'The dataset directory to find the train, validation and test images.')
flags.DEFINE_string('image_val_dir', './dataset', '')
flags.DEFINE_string('anno_val_dir', './dataset', '')
...
flags.DEFINE_string('combine_list', '', 'combine_list')
...
anno_val_dir = FLAGS.anno_val_dir
logdir = FLAGS.logdir

# custom
combine_list = FLAGS.combine_list

if combine_list == '':
   combine_list = None
else:
   load_combine_list(combine_list)
...
annotation_files = sorted([os.path.join(anno_dir, file) for file in os.listdir(anno_dir) if file.endswith('.png')])

image_val_files = sorted([os.path.join(image_val_dir, file) for file in os.listdir(image_val_dir) if file.endswith('.jpg')])
annotation_val_files = sorted([os.path.join(anno_val_dir, file) for file in os.listdir(anno_val_dir) if file.endswith('.png')])
...
#Inverse weighing probability class weights
    ...
    class_weights = np.array([1, 1, 1, 1, 0], dtype=np.float32)
...
def run():
    with tf.Graph().as_default() as graph:
        ...
        #annotation = tf_combine_annotation(annotation)
        ...
        #annotation_val = tf_combine_annotation(annotation_val)
